...
\begin{abstract}
  We study the complexity of SVGD which is an algorithm to sample from $\pi(x) \propto \exp(-F(x))$ where $F$ $L$-smooth. 
\end{abstract}
\section{Introduction}
Sampling has become a prerequisite of many machine learning algorithms. We study SVGD in the population limit.
...
In this paper, we study SVGD in the population limit assuming that the target distribution satisfies Talagrand's inequality T1. We show that $\tilde{\cO}\left(\frac{L{d}^{3/2}}{\lambda^2\varepsilon}\right)$\asnote{Add the log terms to compare to Wibisono Vempala} iterations of SVGD are sufficient to achieve $\varepsilon$ accuracy in terms of KSD. We do not make any unverifiable assumption and we also provide a general convergence result for SVGD.


\section{Background}
\asnote{expectation of an element of the rkhs, T1, T2, LSI, strong logconcavity}
We denote the feature map by $\Phi(x) \coloneqq k(x,\cdot)$.
\newcounter{contlist}
\begin{assumplist}
...
\Cref{ass:T1} is called Talagrand's inequality T1 and is satisfied if and only if there exist $a \in \X$ and $\beta > 0$ s.t. $\int \exp(\beta \|x-a\|^2)d\pi(x) < \infty,$ see~\cite[Theorem 22.10]{villani2008optimal}.
...
\end{proof}
Conditions under which $I_{\st}(\mu_n|\pi) \rightarrow_{n \to +\infty} 0 \Rightarrow \mu_n \rightarrow_{n \to +\infty} \pi$ can be found in~\cite{gorham2017measuring}. In particular, sufficient conditions include $\pi$ being distant dissipative (\textit{e.g.}, $\pi$ is a finite Gaussian mixture with common covariance or $F$ is strongly convex outside a compact set) and $k$ being an inverse multiquadratic kernel, \textit{i.e.}, $k(x,y) = (c^2 + \|x-y\|^2)^\beta$ for some $c > 0$ and $\beta \in (0,1)$.
...
that if $\gamma \|g_n\|_{\cH} \leq \frac{\alpha-1}{\alpha B}$. Then,
    ...
    \end{equation*}
    {\color{red} The condition on the step size depends on $n$. We could get rid of $n$ as in the proof of SVGD (section above) by proving that $\|g_n\|_{\cH} \leq \phi(\E\cF(\mu_n))$, where $\phi$ is a non decreasing function. But this is not true in general, we can only have $\E \|g_n\|_{\cH} \leq \phi(\E \cF(\mu_n))$ using Talagrand's inequality as in the last section. \textbf{So, what bothers us is the fact that the condition on the step size must hold almost surely instead of in expectation}. We can always assume a uniform bound $\|g_n\|_{\cH} < C$ a.s. for every $n$, but we don't want to do that since our final the goal is to study a Variance Reduced version of SVSGD.}
