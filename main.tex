...
\begin{assumption}
\label{ass:T1} 
$\exists \lambda > 0$ s.t. for all $\mu \in \cP_1(\X)$, $W_1(\mu,\pi) \leq \sqrt{\frac{2 \KL(\mu|\pi)}{\lambda}}$.
\end{assumption}
...
\Cref{ass:T1} is rather mild. In particular, using $W_1(\mu,\pi) \leq W_2(\mu,\pi)$, T1 is implied by T2 ($W_2(\mu,\pi) \leq \sqrt{\frac{2 \KL(\mu|\pi)}{\lambda}}$) with the same constant $\lambda$. Moreover, Log Sobolev Inequality (LSI) with constant $\lambda$ implies T2 with the same constant $\lambda$~\cite[Theorem 22.17]{villani2008optimal}. In conclusion, LSI$\Rightarrow$T2$\Rightarrow$T1 with the same constant $\lambda$. Recall that LSI is the key assumption in the analysis of Langevin algorithm in the case where $F$ is nonconvex~\cite{vempala2019rapid}.
