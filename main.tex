...
\begin{assumplist}
	...
	\item 	\label{ass:V_Lipschitz} The Hessian  $H_{f_i}$ is well-defined and $\exists M>0$ s.t. for all $i$, $\|H_{f_i}\|_{\op}\le M$.
...
\begin{theorem}
  Let Assumptions \ref{ass:k_bounded}, \ref{ass:V_Lipschitz} and \ref{ass:T1} hold true. Let $\alpha > 1$ and choose $\gamma > 0$ such that 
  \begin{equation}
    \gamma \leq \min\left(\frac{1}{K + B^2 L_n \sqrt{\frac{1}{p}-1}},\frac{\alpha-1}{\alpha B \|g_n\|_{\cH}}\right),
\end{equation}
where $L_n^2 = \frac{1}{m}\sum_{i=1}^m \left(M+\int \|\nabla f_i(x)\|d\mu_n(x)\right)^2$.
  Then,
  \begin{equation}
    \E \cL_{n+1} \leq
...
\end{equation}
where $L_n^2 = \frac{1}{m}\sum_{i=1}^m \left(M+\int \|\nabla f_i(x)\|d\mu_n(x)\right)^2$.
Define $\cL_n \coloneqq \cF(\mu_n) + \frac{\gamma}{2p}\|g_{n} - h_{\mu_{n}}\|_{\cH}^2$. Using~\eqref{eq:rec1} and~\eqref{eq:rec2},
\begin{align*}
