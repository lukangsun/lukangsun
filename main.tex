...
Sampling is a fundamental task of many machine learning procedures. As sampling algorithms are intended to be applied on large scale problems, it has become more and more important to understand their theoretical properties such as their complexity, as a function of the dimension of the problem $d$ and the accuracy $\varepsilon$ wanted by the user. In this regard, most of the Machine Learning literature has concentrated on understanding the complexity of the Langevin algorithm in various settings, for instance~\cite{jordan,etc.} to name a few. An alternative to the Langevin algorithm is the Stein Variational Gradient Descent (SVGD)~\cite{liu2016stein,liu2017}, that has been applied in several contexts in machine learning, including Reinforcement Learning~\cite{}, blabla and blabla. 
...
\begin{itemize}
    \item A convergence rate in terms of the Kernelized Stein Discrepancy (KSD, see below) without making any unverifiable assumption on the trajectory of the algorithm, but by assuming that the target distribution satisfies the mildest of the Talagrand's inequalities, called T1~\cite{villani2003topics},
