...
\title{Provable Acceleration of Stein Variational Stochastic Gradient Descent via Variance Reduction}
...
\end{abstract}
\section{Background}
\asnote{expectation of an element of the rkhs}
...
\begin{assumplist}
	...
	\item 	\label{ass:V_Lipschitz} The Hessian  $H_F$ of $F=-\log\pi$ is well-defined and $\exists M>0$ s.t. $\|H_F\|_{\op}\le M$.
	%Assume  that $\nabla \log \pi$ is $M$-Lipschitz : \\$\|\nabla \log \pi (x) - \nabla \log \pi(y)\|\le M \|x-y\|$ for any $x,y\in \X$.
	\item \label{ass:T1} $\exists \lambda > 0$ s.t. for all $\mu \in \cP_1(\X)$,\asnote{Actually, we only need it for all $\mu \in \cP_2(\X)$} $W_1(\mu,\pi) \leq \lambda \sqrt{\KL(\mu|\pi)}$.
...
\begin{equation}
    ...
    \begin{array}{ll}
        ...
        v_n + P_{\mu_{n+1}} \nabla f_i - P_{\mu_{n}} \nabla f_i & \mbox{with probability } 1-p, 
    \end{array}
\right.
\end{equation}
...
We start by applying Proposition~\ref{prop:TL}: if $\gamma \|g_n\|_{\cH} \leq \frac{\alpha-1}{\alpha B}$. Then,
    ...
    \end{equation}
One can check by induction that for every $n$, $\E g_n = h_{\mu_n}$. In other words, the stochastic gradient $g_n$ is unbiased. Therefore, the quantity $s_n = \E \|g_n - h_{\mu_n}\|_{\cH}^2$ can be seen as a variance. Moreover, by conditioning
\begin{align*}
    s_{n+1} =& p \E \left(\|P_{\mu_{n+1}} \nabla F - \int \nabla \Phi(x)d\mu_{n+1}(x) - h_{\mu_{n+1}}\|_{\cH}^2\right) \\
...
we have
\begin{align*}
    s_{n+1} &= (1-p)\E \left(\|v_n + P_{\mu_{n+1}} \nabla f_i - P_{\mu_{n}} \nabla f_i - P_{\mu_{n+1}} \nabla F\|_{\cH}^2\right)\\
    &= (1-p)\E \left(\left\|\underbrace{\left(v_n - P_{\mu_{n}} \nabla F\right)}_{= X} + \underbrace{\left(P_{\mu_{n+1}} \nabla f_i - P_{\mu_{n}} \nabla f_i\right)}_{=a_i} - \underbrace{\left(P_{\mu_{n+1}} \nabla F - P_{\mu_{n}} \nabla F\right)}_{=\bar{a}}\right\|_{\cH}^2\right).
\end{align*}
Note that $\bar{a} = \frac{1}{m} \sum_{i=1}^m a_i$, therefore $\frac{1}{m}\sum_{i=1}^m \|a_i - \bar{a}\|_{\cH}^2 \leq \frac{1}{m}\sum_{i=1}^m \|a_i\|_{\cH}^2$. By expanding the square and conditioning,
\begin{align*}
    s_{n+1} &= (1-p)\E \left(\|X\|_{\cH}^2 + \ps{X,a_i - \bar{a}}_{\cH} + \|a_i - \bar{a}\|_{\cH}^2\right)\\
    & =(1-p)\left[\E \|X\|_{\cH}^2 + \frac{1}{m}\sum_{i=1}^m \E \|a_i - \bar{a}\|_{\cH}^2\right]\\
    & \leq (1-p)\left[\E \|X\|_{\cH}^2 + \frac{1}{m}\sum_{i=1}^m \E \|a_i\|_{\cH}^2\right]\\
    & = (1-p)\left[\E \|v_n - P_{\mu_{n}} \nabla F\|_{\cH}^2 + \frac{1}{m}\sum_{i=1}^m \E \|P_{\mu_{n+1}} \nabla f_i - P_{\mu_{n}} \nabla f_i\|_{\cH}^2\right]\\
    & = (1-p) s_n + (1-p) \frac{1}{m}\sum_{i=1}^m \E \|P_{\mu_{n+1}} \nabla f_i - P_{\mu_{n}} \nabla f_i\|_{\cH}^2,\\
\end{align*}
