...
\begin{assumplist}
	...
	\item 	\label{ass:stationary} $\exists x_\star \in \X$ s.t. $\nabla F(x_\star)$ \textit{i.e.}, $F$ admits a stationary point.
...
\asnote{Convergence of SVGD + KSD metricizes weak cv + Anna}
\section{General convergence: IMQ kernel + Distant dissipativity}
...
Using \Cref{ass:V_Lipschitz}, $\|\nabla F(x)\| \leq \|\nabla F(0)\| + M\|x\|$. Therefore, using the triangle inequality for the metric $W_1$,
    ...
    \end{align*}
We conclude using Talagrand's inequality~\Cref{ass:T1}: $W_1(\mu,\pi) \leq \lambda \sqrt{\cF(\mu)}$.
\end{proof}
...
\end{theorem}
\begin{proof}
    The proof is similar to that of Theorem~\ref{th:svgd}. 
...
Second, using Lemma~\ref{lem:svgd} and~\eqref{eq:cond2}, if
\begin{equation}
\label{eq:condition-step-n}
      \gamma \leq (\alpha-1)\left(\alpha B^2 \left(1 + M\lambda \sqrt{\cF(\mu_0)} + M\lambda \sqrt{\cF(\mu_n)} + M W_1(\mu_0,\delta_{x_\star})\right)\right)^{-1},
\end{equation}
then,
    \begin{equation}
    \label{eq:TLproof}
...
\end{proof}
We now prove Corollary~\ref{cor:comp}. Using~\cite[Lemma 1]{vempala2019rapid}, $\cF(\mu_0) \leq F(x_\star) + \frac{d}{2}\log \left(\frac{M}{2\pi}\right)$. Besides,
\begin{equation}
    W_1(\mu_0,\delta_{x_\star}) = \int \|x-x_\star\|d\mu_0(x) = \E_{X \sim \mu_0} \|X - x_\star\| = \frac{1}{\sqrt{M}} \E_{X \sim \mu_0} \|\sqrt{M}(X - x_\star)\|,
\end{equation}
and using the transfer lemma and Cauchy-Schwartz inequality,
\begin{equation}
    W_1(\mu_0,\delta_{x_\star}) = \frac{1}{\sqrt{M}} \E_{Y \sim \cN(0_{\X},I_{\X})} \|Y\| \leq \frac{1}{\sqrt{M}} \left(\E_{Y \sim \cN(0_{\X},I_{\X})} \|Y\|^2\right)^{1/2} = \sqrt{\frac{d}{M}}.
\end{equation}
Therefore, if 
\begin{equation}
    \gamma = (\alpha-1)\left(\alpha B^2 \left(1 + 2M\lambda \sqrt{F(x_\star) + \frac{d}{2}\log \left(\frac{M}{2\pi}\right)} +  \sqrt{Md}\right)\right)^{-1} = \tilde{\cO}\left(\frac{1}{M\lambda\sqrt{d}}\right),
\end{equation}
we have
