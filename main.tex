...
We need $\gamma\|h_{\mu_n}\|_{\cH}\leq \frac{\alpha-1}{\alpha B}$ to bound the Wasserstein Hessian of the $\KL$-divergence. Unlike in the work of \citet{korba2020non}, under Assumption~\ref{ass:T1} we prove that $\gamma$ does not rely on the path information which makes our descent lemma self-contained. 
...
To prove our weak convergence result, we need $(\mu_n)_{n\geq 1}$ to be uniformly tight, which means for every $\epsilon$, there exists a finite radius $R_{\epsilon}$ such that $\limsup_n \mu_n(\|x\|>R_{\epsilon})\leq \epsilon$. Hence,  we need assumptions on the kernel $k(\cdot,\cdot)$ and $\pi$ to enforce uniform tightness. 
...
Next, we provide a convergence rate for the arithmetic mean of the Kernel Stein Discrepancy
	(KSD) $\cO(\frac{1}{n})$ under some assumptions on kernel and $\pi$. This can be further
...
Compared with the Langevin algorithm, whose complexity under LSI  is
	 $\tilde{\cO}(\frac{L^2d}{\lambda^2\epsilon})$, our complexity result $\tilde{\Omega}\left(\frac{L{d}^{3/2}}{\lambda^{1/2}\varepsilon}\right)$ for SVGD, shown in the next corollary,  is better in
	  terms of $\lambda$ and $L$, and moreover, our result applies under T1 only, which is weaker than LSI with the 
	  same constant $\lambda$.  
