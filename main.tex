...
\end{equation}
      ...
      \begin{equation}
      	\begin{split}
      		G_{n+1} &= (1-p) \text{E}\left[\left\|\right.\right. S_{\mu_{n}}\nabla \log \left(\frac{\mu_{n}}{\sigma_{n}}\right) + S_{\mu_{n+1}}\nabla \log \left(\frac{\mu_{n+1}}{\sigma_{I^{\prime}}}\right)-S_{\mu_{n}}\nabla \log \left(\frac{\mu_{n}}{\sigma_{I^{\prime}}}\right)+ \\ &\int\left(k(\phi_{\gamma}(x),\cdot)-k(x,\cdot)\right)\nabla \log \left(\frac{\sigma_n}{\sigma_{I^{\prime}}}\right)\left(x\right)d\mu_n\left(x\right) 
      		-S_{\mu_{n+1}}\nabla \log \left(\frac{\mu_{n+1}}{\pi}\right) \left.\left.\right\|_{H}^{2}\right]\\
      		& = (1-p)\text{E}\left[ \left\|\right.\right. S_{\mu_{n}}\nabla \log \left(\frac{\mu_{n}}{\pi}\right)-S_{\mu_{n}}\nabla \log \left(\frac{\mu_{n}}{\sigma_{n}}\right)\left.\left.\right\|_{H}^{2}\right] \\
      		& + (1-p) \text{E}\left[\left\|\right.\right. S_{\mu_{n}}\nabla \log \left(\frac{1}{\pi}\right) + S_{\mu_{n+1}}\nabla \log \left(\frac{1}{\sigma_{I^{\prime}}}\right)-S_{\mu_{n}}\nabla \log \left(\frac{1}{\sigma_{I^{\prime}}}\right)+ \\ &\int\left(k(\phi_{\gamma}(x),\cdot)-k(x,\cdot)\right)\nabla \log \left(\frac{\sigma_n}{\sigma_{I^{\prime}}}\right)\left(x\right)d\mu_n\left(x\right) 
      		-S_{\mu_{n+1}}\nabla \log \left(\frac{1}{\pi}\right) \left.\left.\right\|_{H}^{2}\right]
      	\end{split}
      \end{equation}
  ...
  \begin{theorem}
    ...
    \begin{equation}
      \bE(\Phi_{n+1})\leq \bE(\Phi_n)-\frac{\gamma_n}{2}\bE\left\|S_{\mu_n}\nabla\log\left(\frac{\mu_n}{\pi}\right)\right\|_H^2,
    \end{equation}
  
  where $\Phi_n:=\KL(\mu_n|\pi)+\frac{\gamma_n}{2p}\left\|S_{\mu_n}\nabla\log\left(\frac{\mu_n}{\pi}\right)-S_{\mu_n}\nabla\log\left(\frac{\mu_n}{\sigma_n}\right)\right\|_H^2$. (In the proof,we use $\gamma$ to replace $\gamma_n$ for simplicity.)
...
\end{equation}
  ...
  We need to know the order of $|k(\phi_{\gamma}(x),\phi_{\gamma}(x^{\prime}))-k(\phi_{\gamma}(x),x^{\prime})-k(x,\phi_{\gamma}(x^{\prime}))+k(x,x^{\prime})|$, in the following we assume supremum norm of the second order derivatives of $k$ are finite, and we miss $\cO(|\phi_{\gamma}(x)-x|^2)$ and $\cO(|\phi_{\gamma}(x^{\prime})-x^{\prime}|^2)$ in the derivation.
  
  \begin{equation}
  	\begin{split}
  		&\quad |k(\phi_{\gamma}(x),\phi_{\gamma}(x^{\prime}))-k(\phi_{\gamma}(x),x^{\prime})-k(x,\phi_{\gamma}(x^{\prime}))+k(x,x^{\prime})|\\
  		& \leq |\langle \phi_{\gamma}\left(x^{\prime}\right)-x^{\prime},\nabla_2 k\left(\phi_{\gamma}\left(x\right),x^{\prime}\right)\rangle-\langle \phi_{\gamma}\left(x^{\prime}\right)-x^{\prime},\nabla_2 k\left(x,x^{\prime}\right)\rangle|\\
  		&\leq |\langle\phi_{\gamma}(x)-x,\nabla_{1,2}^2k(x,x^{\prime})(\phi_{\gamma}(x^{\prime})-x^{\prime})\rangle|\leq C \gamma^2 \left\|S_{\mu_n}\nabla\log(\frac{\mu_n}{\sigma_n})\right\|_H^2
  	\end{split}
  \end{equation}
  ...
  Then, we know,
  \begin{equation}
  \sqN{II} \leq C\left(\int|\nabla\log(\pi(x))|d\mu_n(x)\right)^2\gamma^2\sqN{S_{\mu_n}\nabla\log(\frac{\mu_n}{\sigma_{n}})}
  \end{equation}

So
\begin{equation}
	\left\|S_{\mu_{n+1}}\nabla \log \left(\frac{1}{\pi}\right)-S_{\mu_{n}}\nabla \log \left(\frac{1}{\pi}\right)\right\|_{H}^{2}\leq C_1\left(1+\int|\nabla\log(\pi(x))|d\mu_n(x)\right)^2\gamma^2\sqN{S_{\mu_n}\nabla\log(\frac{\mu_n}{\sigma_{n}})} ,
\end{equation}
  
  similarly, we have 
  
  \begin{equation}
  	\left\|S_{\mu_{n+1}}\nabla \log \left(\frac{1}{\sigma_{I^{\prime}}}\right)-S_{\mu_{n}}\nabla \log \left(\frac{1}{\sigma_{I^{\prime}}}\right)\right\|_{H}^{2}\leq C_2\left(1+\int|\nabla\log(\sigma_{I^{\prime}}(x))|d\mu_n(x)\right)^2\gamma^2\sqN{S_{\mu_n}\nabla\log(\frac{\mu_n}{\sigma_{n}})} ,
  \end{equation}
  ...
  \begin{equation}
   \!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\! \frac{\gamma}{2 p}\left(C_1\left(1+\int|\nabla\log(\pi(x))|d\mu_n(x)\right)^2+C_2\left(1+\int|\nabla\log(\sigma_{I^{\prime}}(x))|d\mu_n(x)\right)^2+C_3\left(\int |\nabla \log\left(\frac{\sigma_n}{\sigma_{I^{\prime}}}\right)|d\mu_n\right)^2\right)-(\frac{1}{2\gamma}-\frac{C}{2})\leq 0,
  \end{equation}
  Then
  \begin{equation}
...
\begin{lemma}
	...
	\begin{equation}
		W_1\left(\mu,\pi\right)\leq C\sqrt{KL\left(\mu|\pi\right)}
	\end{equation}
\end{lemma}
...
\begin{lemma}
		if we have $\Phi_n\leq C_4$, then we have 
		\begin{equation}
			\int|\nabla\log(\pi)(x)|d\mu_n(x)+\int|\nabla\log(\sigma_n)(x)|d\mu_n(x)+\int|\nabla\log(\frac{\sigma_n}{\sigma_{I^{\prime}}})(x)|d\mu_n(x)\leq C_5
		\end{equation}
\end{lemma}
\begin{proof}
    (We ignore the subscripts of the constant.) Since $W_1\left(\mu_n,\pi\right)\leq C\sqrt{KL\left(\mu_n|\pi\right)}\leq C\Phi_n\leq C$, by L-smoothness of $f_i$, we have 
    \begin{equation}
    	\begin{split}
    	&\quad \int|\nabla\log(\pi)(x)|d\mu_n(x)+\int|\nabla\log(\sigma_n)(x)|d\mu_n(x)+\int|\nabla\log(\frac{\sigma_n}{\sigma_{I^{\prime}}})(x)|d\mu_n(x)\\
    	&\leq C(1+\int |x|d\mu_n(x))=C(1+W_1\left(\mu_n,\delta_0\right))\leq C(1+W_1\left(\mu_n,\pi\right)+W_1\left(\pi,\delta\right)\leq C.
    	\end{split}
    \end{equation}
\end{proof}

\begin{lemma}
	If $\Phi\leq A$, then we can choose $\gamma_n\geq \gamma_0$, where $\gamma_0$ is a constant depends on $A$, such that,$\Phi_{n+1}\leq A$(here $A$ is the same with the former $A$. )
\end{lemma}
\begin{proof}
		from the proof of main theorem and the last lemma, we can choose $\gamma_n\geq\gamma_0$, such that $\Phi_{n+1}\leq \Phi_n-\frac{\gamma_n}{2}\left\|S_{\mu_n}\nabla\log(\frac{\mu_n}{\sigma_n})\right\|_H^2\leq A.$
\end{proof}

\begin{lemma}
		if we set $A=\Phi_0$, then we can set $\gamma_n=\gamma_0$, for all $n$, such that
		\begin{equation}
			\Phi_{n+1}\leq\Phi_n-\frac{\gamma_0}{2}\left\|S_{\mu_n}\nabla\log(\frac{\mu_n}{\sigma_n})\right\|_H^2
		\end{equation}
		
\end{lemma}

\begin{proof}
	using the last lemma, it's obvious.
\end{proof}
