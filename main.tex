...
where $g \in \cH$.
\begin{proposition}
\label{prop:TL}\lsnote{we don't need this proposition, lemma1 is enough}
...
\begin{itemize}
    ...
    \item $k$ is an inverse multiquadratic kernel, \textit{i.e.}, $k(x,y) = (c^2 + \|x-y\|^2)^\beta$ for some $c > 0$ and $\beta \in (-1,0)$ (note that \Cref{ass:k_bounded} is satisfied).\lsnote{ assumption 3 is satisfied. $\beta\in[-1,0] or (-1,0)$? in ~\cite[Theorem~8]{gorham2017measuring} it is (-1,0)}
    \asnote{OK! thanks}
\end{itemize}  
