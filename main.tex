...
\begin{tabular}{ |c||c|c|} 
 \hline
 Algorithm  &  Langevin &\color{red}SVGD \\ 
  \hline
 Reference & \citet[Theorem 1]{vempala2019rapid} &\color{red}This paper, Theorem~\ref{th:svgd}  \\
   \hline
 Assumptions & \begin{tabular}{c}LSI with constant $\lambda$\\ $F$ is $L$-smooth \end{tabular} &  \color{red} \begin{tabular}{c}T1 with constant $\lambda$\\ $F$ is $L$-smooth \end{tabular}  \\
   \hline
Criterion & $\KL(\mu|\pi)$ & \color{red}$I_{\st}(\mu|\pi)$ \\
  \hline
Complexity  & $\tilde{\cO}(\frac{L^2 d}{\lambda^2 \varepsilon})$ & \color{red} $\tilde{\cO}\left(\frac{L{d}^{3/2}}{\lambda^{1/2}\varepsilon}\right)$ \\  
 \hline
\end{tabular}
\end{table}
