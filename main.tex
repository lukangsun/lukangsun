...
Since $F$ is continuous, $S$ is closed. It remains to prove that $S$ is bounded. 

Assume that $S$ is not bounded. Then, there exists a sequence $(x_n)$ of points in $\X$ such that $F(x_n) \leq C$, $\|x_n\| \to +\infty$ and $B(x_n) \cap B(x_m) = \emptyset$ for every $n \neq m$, where $B(x)$ denotes and the unit ball centered at $x$. 
...
\end{align*}
where we used Jensen's inequality for the uniform distribution over $B(0)$, thanks to the convexity of $\exp$. Finally, 
\begin{equation*}
\int \exp(-F(x))dx \geq \sum_{n = 0}^{\infty} \int_{B(x_n)} \exp(-F(x))dx \geq \sum_{n = 0}^{\infty} V \exp \left(-C-\frac{L}{2}\right) = +\infty.
\end{equation*}
and $\exp(-F)$ is not integrable. This contradicts the definition of $F$. Therefore, $S$ is bounded.

Finally, the set $S$ is compact and $F$ is coercive. Consequently, $F$ admits a stationary point.

Indeed, $F$ is continuous over the compact set $\{x \in \sX : F(x) \leq 1\}$, therefore, $F$ admits a minimizer $x_\star$ over this set. Moreover, this point $x_\star$ is a stationary point \textit{i.e.}, $\nabla F(x_\star) = 0$ (note that the point $x_\star$ is actually a global minimizer of $F$).
