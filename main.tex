...
%\peter{add some text}
\Sun{We provide the convergence rate for the arithmetic mean of the Kernel Stein Discrepancy
	(KSD) which is $\cO(\frac{1}{n})$, under some assumptions on kernel and $\pi$, this can further
	 used to prove the weak convergence of $\mu_n$. However we can not guarantee the convergence under $KL-divergence$. }
...
%\peter{add some text}
\Sun{Comparing with Langevin algorithm(under LSI and complexity is
	 $\tilde{\cO}(\frac{L^2d}{\lambda^2\epsilon})$), our complexity result for SVGD is better in
	  terms of $\lambda$ and $L$ and our result is only under T1 which is weaker than LSI with the 
