...
\begin{abstract}
  We study the complexity of SVGD which is an algorithm to sample from $\pi(x) \propto \exp(-F(x))$ where $F$ smooth and nonconvex. We provide a clean complexity bound for SVGD in the population limit in terms of the Stein Fisher Information (or squared Kernelized Stein Discrepancy), as a function of the dimension of the problem $d$ and the desired accuracy $\varepsilon$, without assumption on the trajectory of the algorithm. Our key assumption is that the target distribution satisfies Talagrand's inequality T1.
\end{abstract}
...
\begin{equation}
    ...
    \pi(x) \propto \exp(-F(x)),
\end{equation}
where $F:\X \to \R$ is a smooth nonconvex function defined on $\X \coloneqq \R^d$.
...
Stein Variational Gradient Descent (SVGD)~\cite{liu2016stein,liu2017stein} is an alternative to the Langevin algorithm that has been applied in several contexts in machine learning, including Reinforcement Learning~\cite{liu2017policy}, sequential decision making~\cite{zhang2018learning,zhang2019scalable}, Generative Adversarial Networks~\cite{tao2019variational}, Variational Auto Encoders~\cite{pu2017vae}, Federated Learning~\cite{kassab2020federated} etc. However, the theoretical understanding of SVGD is limited compared to that of Langevin algorithm~\cite{lu2019scaling,duncan2019geometry,liu2017stein,chewi2020svgd,nusken2021stein}. In particular, the first complexity result of SVGD appeared recently~\cite[Corollary 6]{korba2020non} under an assumption on the trajectory of the algorithm which cannot be checked prior to running the algorithm. Basically, this assumption is that the first moment of the iterates remain uniformly bounded by a constant $C$ at every iteration. Moreover, the complexity bound~\cite[Corollary 6]{korba2020non} does not explicit the dependence in the dimension $d$ of the problem, precisely because this bound depends on $C$.

Our paper intends to provide a clean analysis of SVGD, without assumption on the trajectory and with complexity bounds depending on $\varepsilon$ and $d$. Our key assumption is that $\pi$ satisfies T1, the mildest of the Talagrand's inequalities, which is satisfied under mild assumptions on the tail of the distribution~\cite[Theorem 22.10]{villani2008optimal} and is implied by the Logarithmic Sobolev Inequality (LSI)~\cite[Theorem 22.17]{villani2008optimal}. Under T1 inequality, 
\begin{itemize}
    \item We provide a convergence rate for SVGD in the so-called population limit without assumption on the trajectory of the algorithm,
    \item We provide a complexity bound for SVGD in terms of the dimension $d$ and the desired accuracy $\varepsilon$,
    \item We provide a generic weak convergence result for SVGD. 
\end{itemize}
All these results hold without assuming $F$ convex. Our main complexity result is summarized in Table~\ref{tab:result}.
\begin{table}[h!]
\centering
\begin{tabular}{ |c|c|c|c|c| } 
 ...
 \hline \hline
 SVGD & This paper, Theorem~\ref{th:svgd} & T1 with constant $\lambda$, $F$ $L$-smooth & $I_{\st}(\mu|\pi)$ & $\tilde{\cO}\left(\frac{L{d}^{3/2}}{\lambda^{1/2}\varepsilon}\right)$\\ 
 \hline
...
\end{tabular}
\label{tab:result}
\caption{Complexity results. Recall that LSI implies T1 with the same constant $\lambda$~\cite[Theorem 22.17]{villani2008optimal}.}
\end{table}
% %HERE
...
\end{assumption}
To specify the dependence in the dimension $d$ of our complexity bounds, we shall sometimes assume the following.
\begin{assumption}
\label{ass:stationary} 
$\exists x_\star \in \X$ s.t. $\nabla F(x_\star) = 0$ \textit{i.e.}, $F$ admits a stationary point.
\end{assumption}
...
LSI is the key assumption in the analysis of Langevin algorithm in the case where $F$ is not convex~\cite{vempala2019rapid}.

\begin{definition}[Talagrand's Inequality T$p$]
Let $p \geq 1$. The distribution $\pi$ satisfies the Talagrand's Inequality T$p$ if $\exists \lambda > 0$ s.t. for all $\mu \in \cP_p(\X)$, $W_p(\mu,\pi) \leq \sqrt{\frac{2 \cF(\mu)}{\lambda}}$.
\end{definition}
...
\textit{In the population limit, SVGD can be seen as a Riemannian gradient descent, thanks to the two following reasons.} 
...
Let Assumptions \ref{ass:V_Lipschitz}, \ref{ass:T1} and \ref{ass:k_bounded} hold true. Assume moreover that $I_{\st}(\mu_n|\pi) \rightarrow_{n \to +\infty} 0 \Rightarrow \mu_n \rightarrow_{n \to +\infty} \pi$ weakly. Let $\alpha > 1$. 
  If 
  \begin{equation*}
      \gamma \leq (\alpha-1)\left(\alpha B^2 \left(1 + \|\nabla F(0)\| + L \int \|x\|d\pi(x) + L \sqrt{\frac{2\cF(\mu_0)}{\lambda}} \right)\right)^{-1}\quad \text{and} \quad \gamma < \frac{2}{B (\alpha^2 + L)},
  \end{equation*}
...
\begin{itemize}
    ...
    \item $k$ is an inverse multiquadratic kernel, \textit{i.e.}, $k(x,y) = (c^2 + \|x-y\|^2)^\beta$ for some $c > 0$ and $\beta \in (0,1)$ (note that \Cref{ass:k_bounded} is satisfied).\asnote{Check this!}
\end{itemize}  
...
Let Assumptions \ref{ass:k_bounded}, \ref{ass:V_Lipschitz} and \ref{ass:T1} hold true. Let $\alpha > 1$. 
  If 
  \begin{equation*}
      \gamma \leq \min\left((\alpha-1)\left(\alpha B^2 \left(1 + \|\nabla F(0)\| + L \int \|x\|d\pi(x) + L \sqrt{\frac{2\cF(\mu_0)}{\lambda}} \right)\right)^{-1},\frac{1}{B (\alpha^2 + L)}\right),
  \end{equation*}
  OR if \Cref{ass:stationary} holds and
  \begin{equation*}
      \gamma \leq \min\left((\alpha-1)\left(\alpha B^2 \left(1 + 2L\sqrt{\frac{2\cF(\mu_0)}{\lambda}} + L \int \|x-x_\star\|d\mu_0(x))\right)\right)^{-1},\frac{1}{B (\alpha^2 + L)}\right),
  \end{equation*}
  then,
  \begin{equation}
...
\label{cor:comp}
Let Assumptions~\ref{ass:V_Lipschitz}--\ref{ass:k_bounded} hold true. Let $\alpha > 1$. If
\begin{equation*}
      \gamma \leq \min\left((\alpha-1)\left(\alpha B^2 \left(1 + 2L\sqrt{\frac{2}{\lambda}}\sqrt{F(x_\star) + \frac{d}{2}\log \left(\frac{L}{2\pi}\right)} + \sqrt{L d}\right)\right)^{-1},\frac{1}{B (\alpha^2 + L)}\right),
  \end{equation*}
...
Most of the existing results on SVGD deal with the \textit{continuous time} approximation of SVGD (in the population limit)~\cite{lu2019scaling,duncan2019geometry,liu2017stein,nusken2021stein}, or with an approximation of that continuous time dynamics~\cite{chewi2020svgd}. In particular, \cite{duncan2019geometry} proposes a Stein logarithmic Sobolev inequality that implies the linear convergence of the continuous time dynamics. However, it is not yet understood when this inequality holds. Besides, \cite{chewi2020svgd} showed that the Wasserstein gradient flow of the chi-squared divergence can be seen as an approximation of the continuous dynamics behind SVGD, and showed linear convergence of the Wasserstein gradient flow of the chi-squared under Poincaré inequality. Other results~\cite{lu2019scaling,liu2017stein,nusken2021stein} include asymptotic convergence properties of the continuous time SVGD, but do not include convergence rates.


To our knowledge, the only existing complexity result (\textit{i.e.}, convergence rate) for SVGD in \textit{discrete time} is~\cite{korba2020non}. They showed that, in the population limit, $\cO(\frac{L}{\varepsilon})$ iterations of SVGD algorithm are sufficient to achieve $\varepsilon$ accuracy in terms of the Stein Fisher Information. However, this complexity result does not express the dependence in the dimension $d$. More importantly, this complexity result is established under the assumption that $\sup_n I_{\st}(\mu_n|\pi) < \infty$, and the quantity $C = \sup_n I_{\st}(\mu_n|\pi)$ appears in the hidden constants. In particular, the dependence of $C$ in $d$ and other parameters is not known.

Finally, the ML literature on the complexity of sampling from a non logconcave target distribution has many focused on the Langevin algorithm. In particular, \cite{vempala2019rapid} showed that $\tilde{\cO}(\frac{L^2 d}{\lambda^2 \varepsilon})$ iterations of Langevin algorithm are sufficient to achieve $\varepsilon$ accuracy in terms of KL divergence, if the target distribution satisfies LSI, which is stronger than T1, see Table~\ref{tab:result}.
...
\begin{lemma}
\label{lem:T1}
     Let Assumptions \ref{ass:V_Lipschitz}, \ref{ass:T1} and \ref{ass:k_bounded} hold true. Then, for every $\mu \in \cP_2(\X)$,
\begin{equation*}
...
\begin{equation*}
\|h_\mu\|_{\cH} \leq B \left(1 + L\lambda \sqrt{\cF(\mu_0)} + L\lambda \sqrt{\cF(\mu)} + L \int \|x-x_\star\|d\mu_0(x)\right).
\end{equation*}
\end{lemma}
\begin{proof}
...
Using \Cref{ass:V_Lipschitz}, $\|\nabla F(x)\| \leq \|\nabla F(0)\| + L\|x\|$. Therefore, using the triangle inequality for the metric $W_1$,
    \begin{align*}
        ...
        &= B \left(1 + \|\nabla F(0)\| + L W_1(\mu,\delta_0) \right)\\
...
Now, assume that the condition~\eqref{eq:condition-step} implies Inequality~\eqref{eq:TL-svgd-cst} for every $n \in \{0,\ldots,N-1\}$ and let us prove it for $n = N$. First, $\cF(\mu_N) \leq \cF(\mu_0)$. Moreover,
\begin{align*}
    &B \left(1 + \|\nabla F(0)\| + L \int \|x\|d\pi(x)\right) + BL \lambda \sqrt{\frac{2\cF(\mu_N)}{\lambda}}\\ \leq &B \left(1 + \|\nabla F(0)\| + L \int \|x\|d\pi(x)\right) + BL \sqrt{\frac{2\cF(\mu_0)}{\lambda}}. 
\end{align*}
Therefore, if $\gamma > 0$ satisfies \eqref{eq:condition-step}, then $\gamma \|h_{\mu_N}\|_{\cH} \leq \frac{\alpha-1}{\alpha B}$. To see this, using Lemma~\ref{lem:T1}
\begin{align*}
\gamma \|h_{\mu_N}\|_{\cH}
&\leq  \gamma \left(B \left(1 + \|\nabla F(0)\| + L \int \|x\|d\pi(x)\right) + BL \lambda \sqrt{\cF(\mu_N)} \right) \\
       &\leq \gamma \left(B \left(1 + \|\nabla F(0)\| + L \int \|x\|d\pi(x)\right) + BL \lambda \sqrt{\cF(\mu_0)} \right) \\
...
\asnote{Check this proof (should be OK)}
...
\section{FOR AFTER NEURIPS: STOCHASTIC GRADIENT}
...
\subsection{Appendix: Lukang's Draft}
